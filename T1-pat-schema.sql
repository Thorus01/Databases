...
ALTER TABLE trip
    ADD CONSTRAINT trip_uq UNIQUE (
        trip_int_pickupdt, trip_int_dropoffdt, veh_vin, driver_id
    );

-- Add all missing FK Constraints below here

ALTER TABLE official
    ADD CONSTRAINT countryregion_cr_ioc_code_fk FOREIGN KEY (
        cr_ioc_code
    )
...
ALTER TABLE official
    ADD CONSTRAINT official_off_cdm_fk FOREIGN KEY (
        off_cdm
    )
        REFERENCES official (
            off_id
...
ALTER TABLE vehicle
    ADD CONSTRAINT vehiclemodel_vm_model_id_fk FOREIGN KEY (
        vm_model_id
    )
...
ALTER TABLE trip
    ADD CONSTRAINT location_pickup_locn_id_fk FOREIGN KEY (
        pickup_locn_id
    )
        REFERENCES location (
            locn_id
...
ALTER TABLE trip
    ADD CONSTRAINT location_dropoff_locn_id_fk FOREIGN KEY (
        dropoff_locn_id
    )
        REFERENCES location (
            locn_id
...
ALTER TABLE trip
    ADD CONSTRAINT language_lang_iso_code_fk FOREIGN KEY (
        lang_iso_code
    )
        REFERENCES language (
            lang_iso_code
...
ALTER TABLE trip
    ADD CONSTRAINT official_off_id_fk FOREIGN KEY (
        off_id
    )
        REFERENCES official (
            off_id
